...
//  }
  {
    const auto y2 = SDD(1, {0}, SDD(0, {0}, sdd::one<conf>()));
    const auto z2 = SDD(1, {1}, SDD(0, {0}, sdd::one<conf>()));

    std::ofstream filey("/Users/hal/Desktop/y2.dot");
    filey << sdd::tools::dot(y2);
    std::ofstream filez("/Users/hal/Desktop/z2.dot");
    filez << sdd::tools::dot(z2);
  }
